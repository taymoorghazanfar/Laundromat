...
public class CurrentOrdersRecyclerAdapter
        extends RecyclerView.Adapter<CurrentOrdersRecyclerAdapter.ViewHolder> implements Filterable {

    private List<Order> orders;
    private List<Order> ordersFull;
    private ICurrentOrderClickListener currentOrderClickListener;
    ...
    public CurrentOrdersRecyclerAdapter(List<Order> orders) {
        this.orders = orders;
    ...
    public void setCurrentOrderClickListener(ICurrentOrderClickListener currentOrderClickListener) {
        this.currentOrderClickListener = currentOrderClickListener;
    }
    ...
    @Override
    public void onBindViewHolder(@NonNull CurrentOrdersRecyclerAdapter.ViewHolder holder, int position) {

        Order order = orders.get(position);

        holder.textViewLaundryName.setText(order.getLaundryName());

        String text = "Order ID: " + order.getId().substring(order.getId().length() - 10)
    ...
    class ViewHolder extends RecyclerView.ViewHolder {
        ...
        public ViewHolder(@NonNull View itemView) {
            ...
            itemView.setOnClickListener(view -> {
                ...
                if (currentOrderClickListener != null
                        && position != RecyclerView.NO_POSITION) {

                    Order order = orders.get(position);

                    currentOrderClickListener.onCurrentOrderClick(order);
                }
            });
        }
    }
